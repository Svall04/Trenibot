...
async def invia_ritardi_giornalieri(context: CallbackContext, chat_id, numero_treno):
    try:
        info = n_info(numero_treno)
        informazioni_treno = retrive(info, 0)  # La funzione ora restituisce una lista
        messaggio = (
            f"Il treno {numero_treno} in arrivo da {informazioni_treno[0]} con arrivo programmato a {informazioni_treno[1]} alle {informazioni_treno[2]} è in ritardo di {informazioni_treno[3]} minuti.\n"
        )
...
# Funzione per ripristinare i reminder all'avvio del bot
    ...
    if os.path.exists(file):
        with open(file, "r") as f:
            ...
            for row in reader:
                ...
                ora_split = ora.split(":")
                trigger = CronTrigger(hour=int(ora_split[0]), minute=int(ora_split[1]))
                scheduler.add_job(invia_ritardi_giornalieri, trigger, args=[application, chat_id, numero_treno])
